...
class HomeController extends Controller
{
    public function index()
    {
        if (\Auth::check()) {
            if (\Auth::user()->type == 'super admin') {
                ...
                $totalService=Service::where('parent_id', parentId())->sum('total_amount');
                $totalFuel=Fuel::where('parent_id', parentId())->sum('total_amount');
                $totalExpense=Expense::where('parent_id', parentId())->sum('amount');
                $result['totalExpense'] = $totalService+$totalFuel+$totalExpense;
    ...
    public function incomeExpenseByMonth()
    {
        ...
        while ($currentdate <= $end) {
            ...
            $totalService=Service::where('parent_id', parentId())->whereMonth('service_start_date', $month)->whereYear('service_start_date', $year)->sum('total_amount');
            $totalFuel=Fuel::where('parent_id', parentId())->whereMonth('date', $month)->whereYear('date', $year)->sum('total_amount');
            $totalExpense=Expense::where('parent_id', parentId())->whereMonth('date', $month)->whereYear('date', $year)->sum('amount');
            $payment['expense'][] = $totalService+$totalFuel+$totalExpense;
            $currentdate = strtotime('+1 month', $currentdate);
        }
